...
#or maybe I should do the sum of blue and not blue?
Gsumtest.prep= b %>%
  group_by(Social_Cue, pre.post) %>%
  summarise(sumBlue=sum(blue), sumNotBlue=sum(not.blue))
Gsumtest.prep = mutate(Gsumtest.prep, BlueProp=sumBlue/(sumBlue+sumNotBlue))
...
SI.glmm=glmer(cbind(blue,not.blue)~pre.post*Social_Cue+(1|bee) + (1|Colony), family=binomial, data=just.naive)
summary(SI.glmm)
Anova(SI.glmm)
...
Day1OrderPlot=ggplot(data=Spread.Day1order, aes(group=Social_Cue, color=Social_Cue, x=Order, y=BluePref)) + facet_grid(.~Neonic) + geom_point(position=pd, size=1) + geom_line(aes(group=Social_Cue), position=pd) +theme_classic()  + labs(title="Bees' Preference for Blue Flowers on Day 1")
Day1OrderPlot
...
SameDay1OrderPlot=ggplot(data=Spread.Day1order, aes(group=Social_Cue, color=Social_Cue, x=Order, y=BluePref)) + facet_grid(.~Neonic) + geom_point(position=pd, size=1) + geom_line(aes(group=Social_Cue), position=pd) +theme_classic() + coord_cartesian(xlim = c(1,5))+ labs(title="Bees' Preference for Blue Flowers on Day 1, Choices 1-5")
SameDay1OrderPlot
...
bsum.colordata=select(bcolordata, bee, Neonic, Treatment_Type, reward.x, pre.post, new.reward)
#sum.colordata
#sum.colordata is the rawest data that shows all of and each bee-flower visit.
#colorcast=dcast(sum.colordata, bee + Treatment_Type+pre.post~reward.x+new.color, fun.aggregate = sum)
#trying to just cast without melting did not work, even when I removes "reward.x"
...
#casting data
bsum.colorcast=dcast(bsum.colormelt, bee +  Neonic+ Treatment_Type+pre.post~new.reward, fun.aggregate = sum)
#sum.colorcast
head(bsum.colorcast)
...
#GLMing data
allblueGLM=glmer(cbind(RewardDay1, Not.RewardDay1)~pre.post*Treatment_Type+(1|bee), family=binomial, data=bsum.colorcast)
summary(allblueGLM)
Anova(allblueGLM)
